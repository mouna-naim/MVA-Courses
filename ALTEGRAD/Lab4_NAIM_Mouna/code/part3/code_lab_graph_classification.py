...
def graphlet_kernel(Gs_train, Gs_test, n_samples=200):
    ...
    for i, G in enumerate(G_train):
        for _ in range(n_samples):
            set_nodes = np.random.choice(G.nodes(), 3, replace=False)
            sub = G.subgraph(set_nodes)

            for idx, graphlet in enumerate(graphlets):
                if nx.is_isomorphic(sub, graphlet):
                    phi_train[i][idx] += 1
                    break  #Exit if no match is found 
...
##################
K_train_gpk, K_test_gpk = graphlet_kernel(G_train, G_test)
##################
...
accuracy_sp = train_and_evaluate(K_train_sp, K_test_sp, y_train, y_test)
print(f"Accuracy with Shortest Path Kernel: {accuracy_sp:.2f}")

# Evaluate the graphlet kernel
accuracy_gpk = train_and_evaluate(K_train_gpk, K_test_gpk, y_train, y_test)
print(f"Accuracy with Graphlet Kernel: {accuracy_gpk:.2f}")
##################
